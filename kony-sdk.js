/*
  * kony-sdk-plain-js Version 8.1.6
  */
...
kony.logger.createNewLogger = function(loggerName, loggerConfig) {
   ...
   logMessage = function(loggerObj,logLevel, msg, params) {
        parseMessage = function(logLevel, msg, params) {
            ...
            if (logLevel.value >= kony.logger.currentLogLevel.value) {
                ...
                var formattedMessage = "["+loggerObj.loggerName+"][" + level + "][" + date + "][" + time + "] : " + msg +" "+ params;
                console.log(formattedMessage);
            }
        };
        parseMessage(logLevel, msg, params);
    };
...
function IntegrationService(konyRef, serviceName) {
	...
	var sessionID = dataStore.getItem("konykonyUUID");
	...
	function _invokeOperation(operationName, headers, data, isRetryNeeded, successCallback, failureCallback, options) {
		...
		if (kony.sdk.cacheID) {
			requestData.cacheid = kony.sdk.cacheID;
		}
		var token;
		for (var i in konyRef.tokens) {
			if (konyRef.tokens.hasOwnProperty(i) && typeof(i) !== 'function') {
		...
		for (var key in requestData) {
			...
			if (requestData[key]) {
				if(typeof(requestData[key]) === "object"){
				    paramsTable = paramsTable + key + "=" + encodeURIComponent(JSON.stringify(requestData[key]));
				}
				else
				{
					paramsTable = paramsTable + key + "=" + encodeURIComponent(requestData[key]);
				}
			}
		}
        ...
        kony.sdk.logsdk.debug("request data is " + paramsTable);

		networkProvider.post(homeUrl + "/" + operationName,
			paramsTable, defaultHeaders,
			function(res) {
				if (res["cacheid"]) {
					kony.sdk.cacheID = res["cacheid"];
				}
				kony.sdk.verifyAndCallClosure(successCallback, res);
			},
			function(xhr, status, err) {
...
function OAuthHandler(serviceUrl, providerName, appkey, callback, type, noPopup, options, appSecret, serviceDoc, isMFVersionCompatible) {
	var _window = window;
	var _popup = null;
	var urlType = "/" + type + "/";

    if (noPopup && isMFVersionCompatible){
        //Saving the required data in session storage, will be used to validate the identity service once after deeplink is redirected.
        sessionStorage.appkey = appkey;
        sessionStorage.appSecret = appSecret;
        sessionStorage.serviceDoc = JSON.stringify(serviceDoc);
        sessionStorage.providerName = providerName;
        sessionStorage.urlType = type;

        if (options && options["include_profile"])
            sessionStorage.include_profile = options["include_profile"];
    }
	...
	var _listener = function(event) {
		...
		_detachEvent();
		try {
            kony.sdk.logsdk.debug("### AuthService::login received authorization code as " + _contents);
			var headers = {};
	...
	if (noPopup && isMFVersionCompatible){
	    //No popup will be shown in case of oauth2 flow, if noPopup is enabled.
		// Redirection will happen to success_url or the default one specified in the mf console.
        _window.open(url, "_self");
    }else {
        _attachEvent();
        var _popup = _window.open(url);
    }
 ...
 * @param {function} failureCallback - Callback method on failure
 */
function handleDeeplinkCallback(queryparams, successCallBack, failureCallBack){
    if (queryparams && "code" in queryparams){
        if (sessionStorage.appkey && sessionStorage.appSecret && sessionStorage.serviceDoc){
            var konyClient = new kony.sdk();
            var loginOptions = {};
            loginOptions["code"] = queryparams["code"];
            loginOptions["urlType"] = sessionStorage.urlType;

            if (sessionStorage.getItem("include_profile"))
                loginOptions["include_profile"] = sessionStorage.include_profile;

            konyClient.initWithServiceDoc(sessionStorage.appkey, sessionStorage.appSecret, JSON.parse(sessionStorage.serviceDoc))
            var authClient = konyClient.getIdentityService(sessionStorage.providerName);
            ...
            failureCallBack({"Result": "Required parameters appkey, appSecret, serviceDoc or urlType not found in sessionStorage"});
        }
    }else{
        failureCallBack({"Result": "Required parameter code is not found the query params."});
    }
}
function konyLogger() {
...
function setLogLevelCallClosure(res, status, xhr, callback){
	kony.sdk.setLogLevelFromServerResponse(xhr.getAllResponseHeaders());
	callback(res);
}
...
function konyNetworkProvider() {
	...
	var konyRef = konyRef || kony.sdk.getCurrentInstance();
	/**
	 * GET Request
	 * @param url
	 * @param data
	 ...
	 * @private
	 */
	this.get = function(url, data, headers, successCallback, failureCallback, options) {
		...
		if(!kony.sdk.isNullOrUndefined(konyRef)) {
			url = konyRef.appendGlobalParams(url,headers,data);
		}
		...
		$.ajax({
			...
			success: function(res,status,xhr){
				setLogLevelCallClosure(res,status,xhr,successCallback);
			},
			error: function(xhr,status,error){
				setLogLevelCallClosure(error, status, xhr, failureCallback)
			}
		});
	 ...
	 * @private
	 */
	this.post = function(url, data, headers, successCallback, failureCallback, options) {
		if (headers == undefined || headers == null) {
			headers = {};
		}
		...
		konyXMLHttpRequest(url, data, headers, 
	 ...
	 * @private
	 */
	this.invokeDeleteRequest = function(url, data, headers, successCallback, failureCallback, options) {
		if (headers == undefined || headers == null) {
			headers = {};
		}
...
function konyXMLHttpRequest(url, params, headers, httpMethod, konyContentType, successCallback, errorCallback, options) {
	...
	//httpRequest.withCredentials = headers["withCredentials"] || false;
...
kony.sdk.setLicenseCall = function(data){
